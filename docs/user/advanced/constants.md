...
| BP_ENABLE_ROOT_PROFILES               | undefined |Setting the value to **`true`** (**Enabled**), will exclude the **`Members component slug`** from the Member's profile URL (e.g. : `site.url/username/` instead of  `site.url/members/username/`.|
